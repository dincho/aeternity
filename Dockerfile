FROM registry.piwo.app/aeternity-builder:latest as builder
...
RUN if [[ $(dpkg --print-architecture) = "arm64" ]] ; then cd /app && GPP_ARCH_FLAGS="-march=native -x c++" make prod-compile-deps ; else cd /app && make prod-compile-deps ; fi
